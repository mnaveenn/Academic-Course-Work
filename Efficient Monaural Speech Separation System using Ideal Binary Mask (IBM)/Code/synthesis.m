...
if nargin < 3
    fRange = [80, 4000]; % default frequency range in Hz
end
if nargin < 4
    winLength = 160;    % default window length in sample points which is 20 ms for 16 KHz sampling frequency
end
if nargin < 5
...
temp1 = gammatoneIBM(mixture, numChan, fRange, fs);          % first pass through gammatone filterbank
