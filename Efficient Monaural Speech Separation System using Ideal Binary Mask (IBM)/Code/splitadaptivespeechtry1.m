...
[gs, gn, gns, gt512, midEarCoeff] = adaptiveanalysisfilterbank(speech, scalednoise, noisyspeech, numChan, ls);

mask = maskcomputation(gs, gn, winLength);

synthesiedspeech = maskmultiplicationandadaptivesynthesis(gns, winLength, midEarCoeff, gt512, mask);

AllOneMask=zeros(numChan, 275);
AllOneMask(:,:)=1;

speechAllOne = maskmultiplicationandsynthesis(gs,winLength, midEarCoeff, gt512, AllOneMask);
