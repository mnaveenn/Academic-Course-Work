function [r cf] = gammatoneIBM(in, numChan, fRange, fs, gL)
...
% gL = 128;          % gammatone filter length or 128 ms for 16 kHz sampling rate

sigLength = length(in);     % input signal length

phase(1:numChan) = zeros(numChan,1);        % initial phases
%IT CAN BE ALSO phase(1:numChan) = zeros(1,1);
...
for i = 1:numChan
    ...
    %loudness-based gain adjustments
